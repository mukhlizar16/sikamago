...
class Mitra_m extends CI_Model {
	...
	public function get_allbimbingan($id)
	{
		$this->db->select('*, m.foto as foto, u.napan as napan, u.nabel as nabel, u.email as email, 
							p.nama_prodi as prodi, m.no_hp as no_hp, r.nama as jenis, d.nama_bidang as bidang,
	...
	{
		$this->db->select('*, b.mhs_id as idm, m.foto as foto_mhs');
		$this->db->from('bimbingan as b');
		$this->db->join('mahasiswa as m', 'm.id = b.mhs_id', 'left');
		$this->db->join('user as u', 'u.id = m.user_id', 'left');
		$this->db->where('b.mhs_id IN (select m.id)', null, false);
	...
	{
		return $this->db->get_where('log', ['mhs_id' => $id]);
	}
	...
	public function detail_pesan($id_pengirim, $id_penerima)
	{
		...
		$this->db->where('penerima_id', $id_penerima);
		$this->db->or_where('pengirim_id', $id_penerima);
		$this->db->group_start();
		$this->db->where('pengirim_id', $id_pengirim);
		$this->db->or_where('penerima_id', $id_pengirim);
		$this->db->group_end();
	...
	{
		return $this->db->get_where('laporan_dosen', ['dosen_id' => $id]);
	}
